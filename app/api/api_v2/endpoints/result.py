...
@router.get("/list/{scan_id}",response_model=List[schemas.Url_Result], status_code=status.HTTP_200_OK)
def get_all(scan_id :int, db_session: Session = Depends(get_db)):
    try:
        url_results = url_crud.get_url_result_by_scan_id(scan_id, db_session)
